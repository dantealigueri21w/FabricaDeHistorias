...
import pe.appmobile.fabricadehistorias.ui.components.MarcadorIlustracion
...
fun GaleriaVisitantesScreen(visitantes: List<VisitanteEntity>) {
    Column(
        modifier = Modifier.fillMaxSize().background(PapelEnvejecido).verticalScroll(rememberScrollState())
    ) {
        EncabezadoDeEstacion(Arte.fondoDeEstacion("galeria"), "Galería de Visitantes")
            ...
            Card(modifier = Modifier.padding(vertical = 4.dp)) {
                Row(modifier = Modifier.padding(16.dp), verticalAlignment = Alignment.CenterVertically) {
                    if (visitante.recibido) {
                        Image(
                            painter = painterResource(Arte.visitante(visitante.id)),
                            contentDescription = visitante.nombre,
                            modifier = Modifier.size(72.dp)
                        )
                    } else {
                        MarcadorIlustracion(
                            etiqueta = "?",
                            descripcionAccesible = "Visitante todavía no recibido",
                            modifier = Modifier.size(72.dp)
                        )
                    }
